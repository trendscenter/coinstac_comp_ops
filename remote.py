...
REMOTE_MANCOVA_PHASES = mpk.MANCOVA_REMOTE
...
if __name__ == "__main__":
    ...
    ut.log(
        "With input %s, and input keys %s"
        % (str(parsed_args.keys()), str(parsed_args["input"].keys())),
        parsed_args["state"],
    )
    ...
    for i, expected_phases in enumerate(PIPELINE):
        if (
            ...
            for operation, args, kwargs in zip(
                ...
                try:
                    ut.log(
                        "Trying operation %s, with args, and kwargs"
                        % (operation.__name__),
                        parsed_args["state"],
                    )
                ...
                ut.log(
                    "Operation output is %s, output keys %s"
                    % (str(parsed_args.keys()), str(parsed_args["output"].keys())),
                    parsed_args["state"],
                )
            if i + 1 == len(PIPELINE):
    ...
    ut.log(
        ...
        % (str(computation_output.keys()), str(computation_output["output"].keys())),
        parsed_args["state"],
    )
